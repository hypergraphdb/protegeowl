...
import gov.miamidade.hgowl.plugin.obsolete.VersionedOntologyComparator.RevisionComparisonResult;
...
public class CompareVersionedOntologyViewPanel extends JPanel
{
	...
	public CompareVersionedOntologyViewPanel(VersionedOntologyComparisonResult result, DistributedOntology dOnto,
			HGPeerIdentity server, String userId)
	{
		this.result = result;
	...
	private void createUI(DistributedOntology dOnto, HGPeerIdentity server, String userId)
	{
		...
		String message = "<html> <h2> Compare Ontology History to Server " + userId + "</h2> " + "<table width='100%' border='0'>"
				...
				+ "<tr><td align='right'><b>Head:</b></td><td>" + VDRenderer.render(dOnto.getVersionedOntology().revision())
				+ "(local)" + "</td></tr>";
		if (!dOnto.getVersionedOntology().changes().isEmpty())
		{
			message += "<tr><td align='right'><b>Pending:</b></td><td> Your local workingset has uncommitted changes (local)"
					+ "</td></tr>";
		}
		message += "</table>";
	...
	public static void showCompareVersionedOntologyDialog(String title, Workspace ws, DistributedOntology dOnto,
			HGPeerIdentity server, String userId, VersionedOntologyComparisonResult result)
	{
