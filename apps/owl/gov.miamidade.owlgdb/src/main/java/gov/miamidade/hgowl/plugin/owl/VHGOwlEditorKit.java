...
public class VHGOwlEditorKit extends HGOwlEditorKit
{
	...
	public boolean handleAddActiveToVersionControlRequest() throws Exception
	{
		...
		if (activeOntology != null)
		{
			if (activeOntology instanceof HGDBOntology)
			{
				...
				if (this.getVersionedRepository().isVersionControlled(
						activeOntology))
				{
					success = false;
					JOptionPane
				...
				else
				{
					...
					getVersionedRepository().addVersionControl(
							(HGDBOntology) activeOntology, user);
					causeViewUpdate();
					success = true;
					JOptionPane
							.showMessageDialog(
	...
	public boolean handleRemoveActiveFromVersionControlRequest()
			throws Exception
	{
		boolean success;
		OWLOntology activeOntology = getActiveOntology();
	...
	public boolean handleCommitActiveRequest() throws Exception
	{
		...
		// PHGDBOntologyManagerImpl hom =
		// (PHGDBOntologyManagerImpl)hmm.getOWLOntologyManager();
		...
		if (vor.isVersionControlled(activeOnto))
		{
			...
			else
			{
				...
				VHGCommitDialog dlg = VHGCommitDialog.showDialog(
						getWorkspace(), vo, activeOnto);
				if (dlg.isCommitOK())
				{
					// DO IT
	...
	// IllegalStateException("Cannot handle delete from repository. No HGOwlOntologyRepository registered with Protege.");
	// // Open Repository delete dialog
	// HGOntologyRepositoryEntry ontologyEntry =
	// VRepositoryViewPanel.showCommitDialog(getWorkspace(), repository);
	...
	public boolean handleRollbackActiveRequest() throws Exception
	{
		...
		if (activeOntology != null)
		{
			...
			if (vo != null)
			{
				...
				{
					RollbackDialog dlg = RollbackDialog.showDialog(
							"Hypergraph Versioning - Rollback", getWorkspace(),
							vo, this);
					if (dlg.isRollbackOK())
					{
	...
	public VersionedOntology getVersionControlledOntologyBy(
			HGOntologyRepositoryEntry ontologyEntry)
	{
		return getVersionedRepository().getVersionControlledOntology(
				ontologyEntry.getOntology());
	}
	...
	public boolean isActiveOntologyVersioned()
	{
		HGOwlModelManagerImpl hmm = (HGOwlModelManagerImpl) getOWLModelManager();
		OWLOntology activeOnto = hmm.getActiveOntology();
		return getVersionedRepository()
				.getVersionControlledOntology(activeOnto) != null;
	}
	...
	public void handleShowHistoryActiveRequest()
	{
		...
		if (vor.isVersionControlled(activeOnto))
		{
			...
			VHGHistoryDialog.showDialog("Hypergraph Versioning - History of "
					+ VDRenderer.render(vo), getWorkspace(), vo, this);
		}
		else
		{
			JOptionPane.showMessageDialog(getWorkspace(),
