...
public class PullDistributedOntologyViewPanel extends JPanel
{
	...
	public PullDistributedOntologyViewPanel(DistributedOntology dOnto, VersionedOntologyComparisonResult result,
			HGPeerIdentity server, String userName)
	{
		this.result = result;
	...
	private void createUI(DistributedOntology dOnto, HGPeerIdentity server, String userName)
	{
		...
		String message = "<html> <h2> Update Ontology changes from the server </h2> "
				...
				+ "<table width='100%' border='0'>" + "<tr><td align='right'><b>Ontology:</b></td><td>" + VDRenderer.render(dOnto)
				+ "</td></tr>" + "<tr><td align='right'><b>Server:</b></td><td>" + VDRenderer.render(server) + " " + userName
				+ "</td></tr>" + "<tr><td align='right'><b>Head:</b></td><td>"
				+ VDRenderer.render(dOnto.getVersionedOntology().revision()) + "(local)" + "</td></tr>" + "</table>";
		...
		add(new JLabel(
				"<html><b>Each green row represents a revision that will be received from the server (remote) to update your local ontology.</b><br\\> "
						+ "Deselecting revisions will only perform update up to the last revision that remains selected. </html>"),
				BorderLayout.SOUTH);
	}
	...
	public static int showUpdateVersionedOntologyDialog(Workspace ws, VersionedOntologyComparisonResult result,
			DistributedOntology donto, String userId, HGPeerIdentity server)
	{
		PullDistributedOntologyViewPanel panel = new PullDistributedOntologyViewPanel(donto, result, server, userId);
		int ret = JOptionPaneEx.showConfirmDialog(ws, "Team - Update " + donto.getWorkingSetData().getOntologyID() + " from "
				+ userId, panel, JOptionPane.PLAIN_MESSAGE, JOptionPane.OK_CANCEL_OPTION, panel.table, new String[] { "Update...",
				"Cancel" }, "Update...");
