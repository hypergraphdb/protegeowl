...
import gov.miamidade.hgowl.plugin.obsolete.VersionedOntologyComparator.RevisionCompareOutcome;
...
public class PullDOTableModel extends AbstractTableModel
{
	...
	{
		if (rowIndex < 0)
			throw new IllegalArgumentException("Rowindex < 0" + rowIndex);
	...
	public Object getValueAt(int rowIndex, int columnIndex)
	{
		...
		if (columnIndex == 0)
		{
			return result.getSource() == null ? result.getTarget().toString() : (result.getSource().toString());
		}
	...
	public String getTooltipAt(int rowIndex, int columnIndex)
	{
		...
		{
			return result.getSource() == null ? null : result.getSource().comment();
		}
		else if (columnIndex == 2)
		{
			return result.getTarget() == null ? "" : result.getTarget().comment();
		}
		else if (columnIndex == 3)
		{
	...
	public String renderRevision(Revision r)
	{
		if (r == null)
			return "";
		return dateF.format(r.timestamp()) + " by " + r.user();
	}
	...
	{
		if (lastSelectedTargetOnlyResultIndex > 0
				&& lastSelectedTargetOnlyResultIndex > comparisonResult.getLastMatchingRevisionIndex())
		{
