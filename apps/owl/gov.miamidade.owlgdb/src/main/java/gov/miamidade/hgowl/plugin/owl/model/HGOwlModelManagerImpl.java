 ...
 * HGModelManagerImpl.
 * Rationale:
 * We copied most of the code here from OWLModelManagerImpl (by Matthew Horridge). 
 * Even though we wanted to make a subclass to it, we found no way to set another ProtegeOWLOntologyManager 
 * as manager instance variable in a subclass. The Fraunhofer OWLDB implementation (v2.0) relied on a setOWLModelManager method. * 
 *  
...
//		//this.
//		//new manager
//		manager = HGOWLManager.createOWLOntologyManager(OWLDataFactoryImpl.getInstance()); //HGManager.createOWLOntologyManager(OWLDataFactoryImpl.getInstance());
...
//        put(AnonymousDefinedClassManager.ID, new AnonymousDefinedClassManager(this));
    ...
    public boolean loadOntologyFromPhysicalURI(URI uri) {
        ...
        try {
            ...
            System.out.println("LOAD FROM PHYSICAL:" + ooce);             // will be handled by the loadErrorHandler, so ignore
        }
    ...
    public void save(OWLOntology ont) throws OWLOntologyStorageException {
        ...
        try{
            ...
            try {
               ...
               //     throw new ProtocolException("Cannot save ontology into Hypergraph: " + physicalURI);
                //}
