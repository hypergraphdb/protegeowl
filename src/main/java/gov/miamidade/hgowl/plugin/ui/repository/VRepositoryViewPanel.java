...
public class VRepositoryViewPanel extends RepositoryViewPanel
{
	...
	public static HGOntologyRepositoryEntry showAddToVersionControlDialog(Component parent, OntologyRepository repository)
	{
		...
		int ret = JOptionPaneEx.showConfirmDialog(parent, "Add Ontology to Version control (" + repository.getName() + ")", panel,
				JOptionPane.WARNING_MESSAGE, JOptionPane.OK_CANCEL_OPTION, panel.getTable());
	...
	public static HGOntologyRepositoryEntry showRemoveFromVersionControlDialog(Component parent, OntologyRepository repository)
	{
		repository.refresh();
		RepositoryViewPanel panel = new RepositoryViewPanel(repository);
		int ret = JOptionPaneEx.showConfirmDialog(parent, "Remove Ontology from Version control (" + repository.getName() + ")",
				panel, JOptionPane.WARNING_MESSAGE, JOptionPane.OK_CANCEL_OPTION, panel.getTable());
		if (ret == JOptionPane.OK_OPTION)
		{
			HGOntologyRepositoryEntry ore = panel.getTable().getSelectedEntry();
			if (ore == null)
	...
	public static HGOntologyRepositoryEntry showCommitDialog(Component parent, OntologyRepository repository)
	{
		repository.refresh();
		RepositoryViewPanel panel = new RepositoryViewPanel(repository);
		int ret = JOptionPaneEx.showConfirmDialog(parent, "Commit Ontology - Version control (" + repository.getName() + ")",
				panel, JOptionPane.WARNING_MESSAGE, JOptionPane.OK_CANCEL_OPTION, panel.getTable());
		if (ret == JOptionPane.OK_OPTION)
		{
			HGOntologyRepositoryEntry ore = panel.getTable().getSelectedEntry();
	...
	public static HGOntologyRepositoryEntry showRollbackDialog(Component parent, OntologyRepository repository)
	{
		repository.refresh();
		RepositoryViewPanel panel = new RepositoryViewPanel(repository);
		int ret = JOptionPaneEx.showConfirmDialog(parent, "Rollback Ontology - Version control (" + repository.getName() + ")",
				panel, JOptionPane.WARNING_MESSAGE, JOptionPane.OK_CANCEL_OPTION, panel.getTable());
		if (ret == JOptionPane.OK_OPTION)
		{
			HGOntologyRepositoryEntry ore = panel.getTable().getSelectedEntry();
	...
	public static HGOntologyRepositoryEntry showRevertOneDialog(Component parent, OntologyRepository repository)
	{
		...
		int ret = JOptionPaneEx.showConfirmDialog(parent,
				"Revert Ontology by one revision - Version control (" + repository.getName() + ")", panel,
				JOptionPane.WARNING_MESSAGE, JOptionPane.OK_CANCEL_OPTION, panel.getTable());
		if (ret == JOptionPane.OK_OPTION)
		{
			HGOntologyRepositoryEntry ore = panel.getTable().getSelectedEntry();
	...
	{
		JOptionPane.showMessageDialog(parent, "You did not select an ontology.", "Hypergraph Versioning - None selected",
